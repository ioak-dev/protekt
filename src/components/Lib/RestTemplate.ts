...
export function httpGet(endpoint: string, headers: any, skipFields: Array<string> = []) {
    return axios.get(baseUrl + endpoint, headers)
        .then(response => {
            if (!unprotectedEndpoints.find((item => endpoint.includes(item)))) {
                response.data = decryptContent(response.data, skipFields);
            }
...
export function httpPost(endpoint: string, payload: any, headers: any, skipFields: Array<string> = []) {
    if (!unprotectedEndpoints.find((item => endpoint.includes(item)))) {
        payload = encryptContent(payload, skipFields);
    }
...
export function httpPut(endpoint: string, payload: any, headers: any, skipFields: Array<string> = []) {
    if (!unprotectedEndpoints.find((item => endpoint.includes(item)))) {
        payload = encryptContent(payload, skipFields);
    }
...
function encryptContent(content, skipFields: Array<string> = []) {
    if (Array.isArray(content)) {
        let encryptedContent: any = [];
        content.forEach(item => encryptedContent.push(encryptObject(item, skipFields)));
        return encryptedContent;
    } else {
        return encryptObject(content, skipFields);
    }

}

function encryptObject(content, skipFields: Array<string> = []) {
...
function decryptContent(encryptedContent, skipFields: Array<string> = []) {
    if (Array.isArray(encryptedContent)) {
        let content: any = [];
        encryptedContent.forEach(item => content.push(decryptObject(item, skipFields)));
        return content;
    } else {
        return decryptObject(encryptedContent, skipFields);
    }

}

function decryptObject(encryptedContent, skipFields: Array<string> = []) {
    let content = {};
