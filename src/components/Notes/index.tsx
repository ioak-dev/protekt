...
class Notes extends Component<Props, State> {
  ...
  search = (event?: any) => {
    ...
    const searchResults = this.state.items.filter(item => {
      if (
        this.state.searchPref.title &&
        match(item.title, this.state.searchtext)
      ) {
        return true;
      } else if (
        this.state.searchPref.tags &&
        match(item.tags, this.state.searchtext)
      ) {
        return true;
      } else if (
        this.state.searchPref.content &&
        match(item.content, this.state.searchtext)
      ) {
        return true;
      }
    });
  ...
  applyFilter = () => {
    ...
    this.state.searchResults.map(item => {
      if (
        isEmptyOrSpaces(this.state.notebookFilter) ||
        this.state.notebookFilter === 'all notebooks' ||
        item.notebook === this.state.notebookFilter
      ) {
        noteList.push(item);
      }
      notebookList.push(item.notebook);
    ...
    noteList = sort(
      noteList,
      this.sortTypes[this.state.sortBy],
      this.state.sortOrder === 'descending' ? true : false
    );
  ...
  render() {
    ...
    const listNoteRef = this.state.view.map(item => (
      <div key={item._id}>
        <NoteRef
          selected={this.state.selectedNoteId === item._id ? true : false}
          id={item._id}
          note={item}
          selectNote={this.selectNote}
          showTag={this.state.notebookFilter === 'all notebooks'}
        />
      </div>
    ));
    return (
      <div className="notes">
        <OakDialog
          visible={this.state.isAddDialogOpen}
          toggleVisibility={this.toggleAddDialog}
        >
          <div className="dialog-body">
            <div>
              <OakSelect
                ...
                elements={this.state.existingNotebookList}
                firstAction="<create new>"
              />
            </div>
            <div>
              {this.state.existingNotebook === '<create new>' && (
                <OakText
                  label="Notebook name"
                  data={this.state}
                  id="newNotebook"
        ...
        <ViewResolver sideLabel="More options">
          <View main>{noteview}</View>
          <View side>
            <div className="filter-container">
              <div className="section-main">
                <Sidebar
                  label="Add New"
                  elements={this.state.sidebarElements['addNew']}
                  icon="add"
                  animate
                />
                <Sidebar
                  label="Search"
                  elements={this.state.sidebarElements['search']}
                  icon="search"
                  animate
                  number={
                ...
                >
                  ...
                  {this.state.isFiltered && (
                    <div className="typography-4 space-top-2">
                      Found {this.state.searchResults.length} notes matching the
                      search criteria
                    </div>
                  )}
                ...
                <Sidebar
                  label={this.state.isFiltered ? 'Search results' : 'All Notes'}
                  icon="notes"
                  number={this.state.view.length}
                >
                  <div className="filter-bar">
                    {this.state.filteredNotebookList.length > 1 && (
                      <div>
                        <OakSelect
                          label="Notebook"
                          data={this.state}
                          id="notebookFilter"
                          handleChange={e => this.handleNotebookFilterChange(e)}
                          elements={this.state.filteredNotebookList}
                          first="all notebooks"
                        />
                      </div>
                    )}
                    ...
                    <div>
                      <OakSelect
                        ...
                        id="sortBy"
                        handleChange={e => this.handleNotebookFilterChange(e)}
                        elements={Object.keys(this.sortTypes)}
                      />
                    </div>
                    <div>
                      <OakSelect
